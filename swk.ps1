...
#
# Use this for initial folder settings
# [Enum]::GetNames('System.Environment+SpecialFolder')
# We choose to start from Desktop
...
# Simple check whether it is a swk file, i.e. its four ASCII digits
if ($SWKDll -match "^\d+$") {
...
#
# Here starts the areas descriptor
#
...
#
# Process OPS version field (not used anymore) just for the sake of i
$OPSVersion = [System.Text.Encoding]::ASCII.GetString($SWKBuffer[($FilePosition)..($FilePosition + ($OldOPSVersionFieldLength - 1))])
...
while ($KeyPosition -lt $KeysFullString.Length) {
  # Get key number
  [int]$KeyNumber = [System.Text.Encoding]::ASCII.GetString($KeysFullString[$KeyPosition..($KeyPosition + $FieldLengthOffset)])
  ...
  # Get license value
  $KeyPosition = $KeyPosition + $NumberOfKeysFieldsLength
  if (($KeyNumber -eq "301") -or ($KeyNumber -eq "302")) {
    ...
    [int]$KeyValue = [System.Text.Encoding]::ASCII.GetString($KeysFullString[$KeyPosition..($KeyPosition + $KeyOffset)])
  }
