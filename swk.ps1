...
<#
$FilePosition = $FilePosition + $CPUIdsElementLength
$CPUIdsElement2Length = "0x" + [System.Text.Encoding]::ASCII.GetString($SWKBuffer[$FilePosition..($FilePosition + $FieldLengthOffset)])
$CPUIdsElement2Length = [int]$CPUIdsElement2Length - $FieldLength
$FilePosition = $FilePosition + $FieldLength
$CPUIdsElement2Contents = [System.Text.Encoding]::ASCII.GetString($SWKBuffer[$FilePosition..($FilePosition + ($CPUIdsElement2Length -1))])
Write-Debug -message "CPU Ids Element 2 lengtn/contents: $CPUIdsElement2Length $CPUIdsElement2Contents"
$CPUIdsElement2Contents = $CPUIdsElement2Contents -split "(.{2})" -ne "" | ForEach-Object {[char][byte]"0x$_"}
$CPUIdsElement2Contents = $CPUIdsElement2Contents -join ""


$CPUIdsElementLength = "0x" + [System.Text.Encoding]::ASCII.GetString($SWKBuffer[$FilePosition..($FilePosition + $FieldLengthOffset)])
$FilePosition = $FilePosition + $CPUIdsElement2Length
#>
...
foreach ( $ID in $CPUIds ) {
   Write-Host "CPUId $CPUIdsCounter : $ID"
   $CPUIdsCounter++
   }
<#
Write-Host "CPUId 1:" $CPUIdsElementContents
 if ( $CPUIdsElementContents.Length -eq $CPUIdsElement2Contents.Length ) {
   Write-Host "CPUId 2:" $CPUIdsElement2Contents
   }
   else {
     Write-Host $CPUIdsElement2Contents
     }
#>
